...
interface AdRejectedEmailRequest {
  email: string;
  name: string;
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { email, name, vehicle_title, rejection_reason }: AdRejectedEmailRequest = await req.json();
    ...
    const emailResponse = await resend.emails.send({
      ...
      to: [email],
      subject: `❌ Anúncio rejeitado: ${displayTitle} — Zé do Rolo`,
      // @ts-ignore - Resend template support
      template: {
        id: "anuncio-rejeitado",
        variables: {
          name: displayName,
          vehicle_title: displayTitle,
          motivo_rejeicao_anuncio: displayReason,
          vehicles_url: vehiclesUrl,
        },
      },
    });

    console.log("Ad rejected email sent successfully:", emailResponse);
    ...
    console.error("Error sending ad rejected email:", errorMessage);
    return new Response(
