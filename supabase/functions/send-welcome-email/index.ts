...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(`Sending welcome email via Resend template to: ${email} (name: ${displayName})`);

    const emailResponse = await resend.emails.send({
      from: "Zé do Rolo <contato@zedorolo.com>",
      to: [email],
      subject: "🚗 Bem-vindo ao Zé do Rolo! Sua conta foi criada",
      // @ts-ignore - template support
      template: {
        id: "welcome-email",
        variables: {
          name: displayName,
    ...
    console.log("Welcome email sent successfully via template:", emailResponse);
